...
class AddCategorieDash extends Component {
  constructor(props) {
    super(props);
    this.state = {
      image: "",
  ...
  initializeCategorie = () => {
    ...
    this.setState({ newCategorie: newCategorie });
  };

  editPic = (e) => {
    let image = URL.createObjectURL(e.target.files[0]);

    let newCategorie = this.state.newCategorie;
    newCategorie.image = e.target.files[0];
    this.setState({
      newCategorie,
      image,
    });
      ...
      >
        {(close) => (
          <div className="rounded-xl shadow-xl text-center py-3">
            ...
            <div className="font-bold text-third"> Détails de catégorie : </div>
            <div className="w-full p-5">
              <table className="w-full text-xs">
                <tbody>
                  <tr className="h-8 border-b-2">
                    <td className="font-bold">Libellé</td>
                    <td>
                      <textarea
                        id="libelle"
                        onChange={(e) => {
                          let newCategorie = this.state.newCategorie;
                          newCategorie.libelle = e.target.value;
            ...
            <div className="actions">
              <button
                ...
                onClick={() => {
                  this.props.ajouterCategorie(this.state.newCategorie);
                  this.initializeCategorie();
                  close();
                }}
              >
...
export default AddCategorieDash;
