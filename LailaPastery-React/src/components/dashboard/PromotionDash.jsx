...
class PromotionDash extends Component {
  state = {
    ...
    >
      {(close) => (
        <div className="rounded-xl shadow-xl text-center py-3">
          ...
          <div className="font-bold text-third"> Suppression du promo : </div>
          <div className="w-full p-5">
            Vous êtes sur que vous voulez supprimer le promo{" "}
          ...
          <div className="actions">
            <button
              ...
              onClick={() => {
                this.props.deletePromotion(this.state.promotion.id);
                close();
              }}
            >
              Supprimer
  ...
  render() {
    return (
      <tr className="border-b-2 text-center">
        <td>{this.state.promotion.id}</td>
        <td>{this.state.promotion.descPromo}</td>
        <td>{this.state.promotion.valeurPromo} %</td>
